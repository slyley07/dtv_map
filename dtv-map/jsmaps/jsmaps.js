...
(function($) {
  ...
  $.fn.JSMaps = function(options) {
    ...
    function loopPins(target, pins, pinsAr, color) {
      for (var g = 0; g < pins.length; g++) {
        animatePaths(pinsAr, [g], '#FFFFFF');
        if (target === pins[g].state) {
          animatePaths(pinsAr, [g], color);
        }
      }
    }

    function findAbbr(states, target, col) {
      for (let t = 0; t < states.length; t++) {
        if (states[t].attrs.text === target.abbreviation) {
          states[t].attr({fill: col});
        }
      }
    }
    ...
    function renderMap() {
      ...
      if (config.stateClickAction === 'text') {
        // Create text div
        textArea = $('<div class="outerInfobox"><svg class="exit" xmlns="http://www.w3.org/2000/svg" width="8" height="8" viewBox="0 0 8 8"><path d="M1.41 0l-1.41 1.41.72.72 1.78 1.81-1.78 1.78-.72.69 1.41 1.44.72-.72 1.81-1.81 1.78 1.81.69.72 1.44-1.44-.72-.69-1.81-1.78 1.81-1.81.72-.72-1.44-1.41-.69.72-1.78 1.78-1.81-1.78-.72-.72z" /></svg><div class="text-container"><div class="jsmaps-text"><div class="inner-text"></div></div></div></div>').appendTo(mapWrapper);
        closeLayer = $('<div class="closeLayer exit"></div>').appendTo(mapWrapper);
        $('.inner-text').html(config.defaultText);
      ...
      $('.exit').on('click', function(event) {
        ...
        for (let j = 0; j < paths.length; j++) {
          if (paths[j].enable) {
            animatePaths(pathsAr, [j], config.blue);
            findAbbr(statesTexts, paths[j], config.abbreviationColor);
          }
        }

        for (let k = 0; k < pinsAr.length; k++) {
          animatePaths(pinsAr, [k], "#FFFFFF");
        }

        $('.outerInfobox').scrollTop(0);
      });
      ...
      function createMap() {
        ...
        var textProperties = {
          'font-family': 'ATTAleckSans_Bd, Arial, sans-serif',
          'font-weight': 'bold',
        ...
        for (var i = 0, len = paths.length; i < len; i++) {
          ...
          var linePathProperties = {
            'stroke-width': config.strokeWidth || 1,
            'fill': paths[i].color,
            'stroke': config.smallStrokeColor,
            'id': i
          };

          if (paths[i].linePath) {
            linePath = r.path(paths[i].linePath).attr(linePathProperties);
            linePathsAr.push(linePath);
          }
          ...
          function hitAreaOverOut(e) {
            ...
            if (!enabled) {
              isMouseover ? showTooltip(target.name + '<br \/>We\u2019re always working to add new locals. Check back soon!') : removeTooltip();
            }
            ...
            if (enabled) {
              ...
              if (!current) {
                //no current
                for (var g = 0; g < pins.length; g++) {
                  if (target.abbreviation === pins[g].state) {
                    animatePaths(pinsAr, [g], config.blue);
                  }
                }

                findAbbr(statesTexts, target, '#000000');
                animatePaths(pathsAr, [id], color);
              } else if (current.abbreviation) {
                //current is a state
                if (current.abbreviation === target.abbreviation) {
                  //current state matches target state
                  for (var g = 0; g < pins.length; g++) {
                    if (target.abbreviation === pins[g].state) {
                      animatePaths(pinsAr, [g], config.blue);
                    }
                  ...
                  //current state does not match target state
                  for (var g = 0; g < pins.length; g++) {
                    if (target.abbreviation === pins[g].state) {
                      animatePaths(pinsAr, [g], config.blue);
                    }
                ...
                animatePaths(pathsAr, [target.id], '#FFFFFF');
                findAbbr(statesTexts, target, '#000000');
              } else if (current.state) {
                //current is a pin

                if (current.state === target.abbreviation) {
                  //pin's state and target match
                  // console.log('pin state and target match');
                } else {
                  //current is a state and does not match target
                  // console.log('pin state and target do not match');
                  for (var g = 0; g < pins.length; g++) {
                    if (target.abbreviation === pins[g].state) {
                      animatePaths(pinsAr, [g], config.blue);
                    }
                  ...
                  animatePaths(pathsAr, [target.id], color);
                  findAbbr(statesTexts, target, '#000000');
                }
              ...
              if (isMouseOut) {
                if (!current) {
                  ...
                  for (let o = 0; o < pins.length; o++) {
                    animatePaths(pinsAr, [o], '#FFFFFF');
                    findAbbr(statesTexts, target, '#FFFFFF');
                  }
                } else {
                  if (current.abbreviation) {
                    //current is a state
                    if (current.abbreviation !== target.abbreviation) {
                      //current does not match the target
                      for (let o = 0; o < pins.length; o++) {
                        if (target.abbreviation === pins[o].state) {
                          animatePaths(pinsAr, [o], '#FFFFFF');
                          animatePaths(pathsAr, [target.id], config.blue);
                          findAbbr(statesTexts, target, '#FFFFFF');
                        }
                      }
                    }
                    ...
                    //current is a pin
                    if (current.state === target.abbreviation) {
                      // console.log('current pin === target state');
                      animatePaths(pathsAr, [target.id], '#FFFFFF');
                      animatePaths(pinsAr, [current.id], '#000000');
          ...
          hitArea.click(function(e) {
            ...
            if (enabled) {
              ...
              if (config.stateClickAction === 'text') {
                var top = target.textPosX;
                var left = target.textPosY;
                var width = 0;
                var height = 0;

                if (winWidth <= 1000 && winWidth > 767) {
                  width = '170px';
                  height = '200px';
                ...
                for (let p = 0; p < pins.length; p++) {
                  animatePaths(pinsAr, [p], '#FFFFFF');
                  if (pins[p].state === target.abbreviation) {
                    animatePaths(pinsAr, [p], config.blue);
                    // current = target;
                  }
                }

                $('.inner-text').html(target.text);
              ...
              if (!current) {
                for (let j = 0; j < pins.length; j++) {
                  if (target.abbreviation === pins[j].state) {
                    animatePaths(pinsAr, [j], config.blue);
                  }
                }
                ...
                if (current === target) {
                  ...
                  for (let h = 0; h < paths.length; h++) {
                    if (paths[h].enable) {
                      animatePaths(pathsAr, [h], config.blue);
                    }

                    if (target.abbreviation === paths[h].abbreviation) {
                      animatePaths(pathsAr, [h], '#FFFFFF');
                    } else {
                      findAbbr(statesTexts, paths[h], '#FFFFFF');
                    }
                  }
      ...
      function layerMap() {
        ...
        linePathsAr.forEach(function(element) {
          element.toFront();
        })
      }
      ...
      function createPins() {
        for (var i = 0; i < pins.length; i++) {
          ...
          function pinOverOut(e) {
            ...
            var isMouseout = e.type === 'mouseout';
            var color = isMouseover ? target.hoverColor : target.color;
            var callback = isMouseover ? settings.onStateOver(target) : settings.onStateOut;

            if (isMouseover) {
              if (!current) {
                for (var k = 0; k < paths.length; k++) {
                  if (target.state === paths[k].abbreviation) {
                    animatePaths(pathsAr, [k], '#FFFFFF');
                    findAbbr(statesTexts, paths[k], config.abbreviationHoverColor);
                  }
                }

                for (var q = 0; q < pins.length; q++) {
                  if (target.state === pins[q].state) {
                    animatePaths(pinsAr, [q], color);
                  }
                }
              } else {
                if (current.abbreviation) {
                  //current is a state
                  if (current.abbreviation !== target.state) {
              ...
              if (!current) {
                for (var k = 0; k < paths.length; k++) {
                  //change enabled states back to blue and abbreviations back to white
                  if (paths[k].enable) {
                    animatePaths(pathsAr, [k], config.blue);
                    findAbbr(statesTexts, paths[k], config.abbreviationColor);
                  }
                }

                for (let f = 0; f < pinsAr.length; f++) {
                  animatePaths(pinsAr, [f], '#FFFFFF')
                }
              } else if (current.abbreviation && current.abbreviation === target.state) {
                for (var k = 0; k < paths.length; k++) {
                  if (paths[k].enable && paths[k].abbreviation === target.state) {
                    animatePaths(pathsAr, [k], '#FFFFFF');
                    findAbbr(statesTexts, paths[k], '#000000');
                  }
                }
              } else if (current !== target && current.state !== target.state){
                for (var k = 0; k < paths.length; k++) {
                  if (paths[k].enable && paths[k].abbreviation === target.state) {
                    animatePaths(pathsAr, [k], config.blue);
                    findAbbr(statesTexts, paths[k], config.abbreviationColor);
              ...
              } else if (current.state && current.state === target.state) {
                animatePaths(pinsAr, [current.id], '#000000');
              }
            }
          ...
          pin.click(function(e) {
            ...
            for (let j = 0; j < paths.length; j++) {
              ...
              if (paths[j].abbreviation === pins[id].state) {
                animatePaths(pathsAr, [j], '#FFFFFF');
                findAbbr(statesTexts, paths[j], config.abbreviationHoverColor);
              }
            }
            ...
            for (let p = 0; p < pins.length; p++) {
              ...
              if (pins[p].state === target.state && target !== current) {
                animatePaths(pinsAr, [p], config.blue);

                for (var s = 0; s < paths.length; s++) {
                  if (paths[s].abbreviation === target.state) {
                    current = paths[s];
                  }
                }
              }
            ...
            if (config.stateClickAction === 'text') {
              ...
              // resizeMap(target.textPosX, target.textPosY, 'block');
      ...
      /////////////////////////////
      function resizeMap(top, left, display) {
      ...
      for (let m = 0; m < linePathsAr.length; m++) {
        animatePaths(linePathsAr, [m], '#000000');
      }
      ...
      function createSelect() {
        ...
        mapSelect.on('change', function() {
          if (this.value !== 'default') {
            ...
            // mapWrapper.trigger('cityClick', this.value)
          }
          else {
    ...
    function createAccordian() {
      for (let k = 0; k < paths.length; k++) {
        ...
        let name = paths[k].name;

        if (name === 'Washington, D.C.') {
          name = 'District of Columbia';
        }

        let acc = document.getElementById('jsmaps-accordian');
        let txt = ''
        txt += '<div class="indState">';
        txt += '<h2 class="stateText">' + name + '</h2><svg class="downArrow" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 8 8"><path d="M0 0l4 4 4-4h-8z" transform="translate(0 2)" /></svg><svg class="upArrow" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 8 8"><path d="M4 0l-4 4h8l-4-4z" transform="translate(0 2)" /></svg></div><ul class="cityList">';

        if (paths[k].enable) {
          for (let u = 0; u < paths[k].channels.length; u++) {
            txt += '<li class="cityName">' + paths[k].channels[u].name + '</li><ul>';
            for (let y = 0; y < paths[k].channels[u].channels.length; y++) {
              txt += '<li class="channelName">' + paths[k].channels[u].channels[y] + '</li>';
            }
            txt += '</ul>';
          }
        } else {
          txt += '<p>We\u2019re always working to add new locals. Check back soon!</p>';
        }
    ...
    $('.indState').on('click', function() {
      let active = '';
      $(this).children('.upArrow, .downArrow').toggle();
      $(this).next('ul.cityList').slideToggle();
    });
    ...
    mapWrapper.on('cityClick', function(_, name) {
      $.each(statesHitAreas, function(index, elem) {
        // pathName = elem.name;
